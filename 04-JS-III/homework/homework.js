...
function mesesDelAño(array) {
  ...
  let meseElegidos = array.includes("Enero","Marzo", "Noviembre",)
  if(meseElegidos === true){
      return nuevoArray
  } else {
...
function mayorACien(array) {
  ...
  let nuevoArray = array.filter(n => n > 101);
  return nuevoArray;
}
...
function breakStatement(numero) {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if(sumatoria === i){
      ...
      nuevArr.push(sumatoria)
    }
    
  }

  if( nuevArr.length === 10){
    return nuevArr;
  }
  else if( nuevArr.length < 10){
